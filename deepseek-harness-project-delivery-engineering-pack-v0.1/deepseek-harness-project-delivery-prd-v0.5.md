# DeepSeek Harness 项目交付工作台 PRD v0.5
...
> v0.5 主要变化：在 v0.4 产品与视觉冻结基础上补充工程身份边界：MVP 中一个 Project Workspace 对应一个 Planning Project；跨 Workspace 的相同外部 Issue 共享 ExternalIdentity，但本地 WorkItem projection 保持 Workspace-scoped，不引入跨 Workspace global aggregate。
...
同一个 Issue 可以属于多个 Project，因此 membership 与内容身份必须始终分离。

MVP 进一步明确：一个 Project Workspace 对应一个 Planning Project。若同一个外部 Issue 出现在两个不同 Project Workspace，外部 `ExternalIdentity` 仍然唯一，但每个 Workspace 可以保留自己的本地 WorkItem projection、ExecutionContext 与关系状态；首个版本不引入跨 Workspace global WorkItem aggregate。这样既保持外部身份正确，又避免不同项目工作区的本地执行控制互相耦合。
