...
test('特权 reconcile 只由默认分支 workflow 运行，并显式 checkout 默认分支', () => {
  ...
  const download = workflow.jobs.sync.steps.find((step) => step.uses?.startsWith('actions/download-artifact@'))
...
test('review signal workflow 无权限、无 secret、无 checkout，只上传 PR 编号', () => {
  ...
  const upload = workflow.jobs.signal.steps.find((step) => step.uses?.startsWith('actions/upload-artifact@'))
