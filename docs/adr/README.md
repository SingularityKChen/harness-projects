...
本目录只登记**已在仓库内有证据**的决策：上表六条里四条来自 Gate E1 的实测记录，ADR-0005 与 ADR-0006 来自 #27 的 schema 评审（PR #122 的两轮），`Proposed` 表示证据已进仓库、采纳权在人类伙伴（`docs/architecture/release-gates.md` §2 第 1 行把 Gate E1 的判定者列为人类伙伴，§1 规定 R1 不得由 LLM 判定；裁决见 `docs/architecture/gate-e1-ruling.md`，该行对 `AGENTS.md` §1.2 的错误归因登记在裁决 §6.5）。
