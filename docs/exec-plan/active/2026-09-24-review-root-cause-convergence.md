...
- (2026-09-26) **级联 rebase 的「整份文件取一边」是静默丢改动的操作，而且它删掉了唯一能发现这件事的检查**：第四轮把 L5/L6 逐层 rebase 到新父时，装配文件（`tests/contract/storage-contract.test.js`）的冲突用 `--theirs`/`--ours` 整份解决，于是被取的那一边的改动**无声消失**——L5 丢掉了 6 条判别性用例（含 P1-a 的空串/undefined 用例与结算令牌用例）、SQLite 的身份同步组注册与装配台账；L6 丢掉了执行组注册，还把 L4 已修的 `replacePlanningProjections` 原子性改回了 `mutate`。**更关键的是：L5 在第四轮加上的「装配台账 + 独立书写的 `EXPECTED_ASSEMBLY`」（`ae39655`）被 L5 自己第四轮之后的改写（`d84bc16`）换成了直接调用（第六轮评审订正：原文把它归给 L4，而 L4 的任何版本都没有这道守卫），那道守卫正是唯一能发现「某个适配器少注册一组」的检查——于是这一整类丢失在套件上表现为全绿**（不注册 SQLite 同步组时 117/117 仍然绿）。第一性原理的两条在这里同时被违反：控制只有在**能执行**的地方才算数（守卫被删了），以及解析不到的输入必须**响亮失败**（少跑 8 条却静默通过）。**处理**：L5 恢复装配台账与守卫、恢复被丢的用例与注册；L6 恢复执行组注册与 `atomic`；并把「级联的冲突必须按**内容**解决，解决完必须跑守卫与注入实验」写进 Decision Log。
- (2026-09-26) **对文档做批量字符串替换会改坏可执行命令**：把 `storage-foundation.js` 替换成带反引号的路径、把 `8f11e38` 替换成 `8f11e38（观察时刻快照，不可复跑）`，结果中文注释被插进了 `git diff --check …` 的参数位置，命令**根本跑不起来**；反引号还嵌套成了 ``` `suites/storage.js`（地基组） ```。这类「检查器能过、命令不能跑」的文本比没有标注更糟——它把一条可复核的证据变成了一条不可执行的断言。**处理**：命令一律用 `BASE=$(gh pr view <n> … -q .baseRefOid)`，快照标注只放在行内说明里，不进命令参数；改完逐条复制执行一次。
...
- **Decision**：级联的冲突一律按**内容**解决（把两边的语义都搬过来），并在解决后跑「装配守卫 + 该层声明的注入实验」；禁止用 `--theirs`/`--ours` 整份取一边。
  **Rationale**：第四轮用整份取一边，静默丢了 6 条判别性用例、两处注册与一处 `atomic`，第五轮评审逐条复现（`storage-sync.ts:170` 的 P1 等 28 条）。同时被删掉的装配台账正是唯一能发现「少注册一组」的检查，所以丢失在套件上表现为全绿——控制必须在能执行的地方才算数。`git rebase` 的冲突解决不是「选一边」，而是「合并两份语义 + 用守卫证明合并完整」。
