...
三个批次全部完成，验收项 1–8 与 10 通过，第 9 项需要合并后的真实事件。改动落在 7 个提交上：计划、Batch 1（workflow + 脚本）、Batch 2（契约测试新增 7 条）、Batch 3（文档与计划回填）、登记 PR #98 与真实 CI 事件的报告证据、对抗验收后的 P2 补测（契约测试再增 2 条）、以及记录 `runs` API 复核陷阱并回填验收结论。
