...
- **Deviation**（2026-09-18 CST，Batch F）：本批在 harness 提供的隔离工作区里执行，路径是 `.claude/worktrees/<name>/`，不是 §7.1 规定的 `.worktrees/<task-slug>/`。按 §7.2 的第一项检查（`git rev-parse --git-dir --git-common-dir`）确认**已经在隔离工作区内**，因此没有再嵌套创建一个——§7.2 本身要求避免嵌套。但 §7.1 写明该路径规则「覆盖任何工具或技能的默认值」，所以这仍是一处偏差，且不是本批能自行消除的（工作区由 harness 在任务开始前建好）。影响限于工作区位置，分支名与提交流程不受影响。**待人类伙伴裁决**：是把 §7.1 增补成「承认 harness 工作区路径」，还是要求 agent 在其中再建一个 `.worktrees/<task-slug>/`。
...
- 2026-09-18 CST：Batch F 补记工作区路径偏差（§7.1 vs harness 默认路径），待裁决。
