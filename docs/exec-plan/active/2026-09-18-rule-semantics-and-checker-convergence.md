...
### Batch F · 评审反馈按根因收敛（PR #56–#60 + #63）

**最小闭环**：13 条行级意见全部处置，且处置方式按**三条根因**而不是逐条打补丁。

**为什么本节只写在 #56 的副本上**：计划文件在六个分支上已经有三种 blob（#56 一种、四个批次分支一种、#63 一种），而「五份逐字节相同」这个错误前提正是评审意见 #4 的内容。继续多分支编辑该文件会把那条缺陷放大。因此 Batch F 只在 #56 上落地，其余分支的提交信息引用它、不碰该文件。

**13 条的判定：全部属实，其中 6 条本人直接复现**（`PROJECTS_TOKEN` 存在性、计划文件三种 blob、cwd 收窄、`home` 误报、遮蔽泄漏、`Auto-close` 推导）。

**唯一一处判定为「部分过严」**：#57 那条的补救清单里，(b)「为批准留痕新建可 grep 格式 + 脚本 + 契约测试」超出 MVP 阶段——它守的是本仓库自身的项目管理流程而非产品不变量，且按 §9.2 这类含人解释的判定只能做 advisory。采纳该意见自己列为最小修法的 (d)，加上 (c)，把 (a)(b) 开成 issue。这不是与评审对立，是取它给出的最小集。

#### RC1 · 散文承诺了代码不做的事（6 条）

这正是本计划要消灭的缺陷类，而五个 PR 把它重新引入了六次——**这条数据本身比任何一条单独的修复都重要**：说明「规则文本与实现一致」不会因为写进计划就自动成立。

修法不是改六句话，而是给「声称机械保证」这件事定一条可核对的形式：

1. 声称机械保证的规则文本，必须点名**执行它的命令**（§9.5 做对了，它写了 `node scripts/workflow-check.mjs`）；
2. 点不出命令的，改写成「人工复核项」并写明**提升为机械检查的前置条件**；
3. 代码能廉价兑现的，**兑现它**而不是弱化承诺。

第 3 条对第 1、2 条不是例外而是优先项：判据是「兑现成本 vs 弱化后的信息损失」。本轮六条里，#9（遮蔽）/#10（`types: edited`）/#11（pathspec）走第 3 条，#3（机械可执行）/#5（W7 非空）走第 2 条，#1（推导）走的是第三种——**修推导本身**，让规则真的覆盖那一行。

#### RC2 · 判定域只覆盖作者想到的形态（5 条）

实现枚举了作者想到的输入形状，测试枚举**同一套**形状，于是未被想到的形状对两者同时不可见。这解释了为什么本轮所有批次都做了变异测试却仍留下这五条：变异测试证明「已写的判定有牙」，证明不了「该判定的域是完整的」。

修法：

1. **fail-closed 默认值必须有自己的测试**——它是所有未枚举形状的兜底，恰恰最不该没有测试（#6）；
2. 输入语言有**外部规范**时（GitHub closing keyword 文档、CommonMark），用例从规范推导而不是凭直觉（#8 #13）；
3. 错误路径要有**分类测试**：每种失败形态 → 它的 exit code（#7）；
4. 正则类判定补**形状约束**而不是继续加豁免名单（#12：要求主机名标签含数字或 `-`，而不是把 `styles` 加进黑名单——名单会无限增长且永远滞后）。

#### RC3 · 把时点外部事实写成永久断言（2 条）

修法：写**不依赖该事实的理由**，或写**重新核验的命令**；结论不建立在存在性断言上。

#2 恰好自证了这一点：删掉「`PROJECTS_TOKEN` 不存在」之后，§7 的结论仍然成立，因为另两条理由与凭据有无无关——那条断言本来就是多余的支撑。

#### 批次划分

按**文件所有权**切，不按根因切：根因是设计输入，不是切分依据；同一文件的多条意见合在一个提交里才能一次验收。

| 子批 | PR | 覆盖的意见 | 根因 |
|---|---|---|---|
| F1 | #60 | 遮蔽泄漏、`types: edited`、cwd pathspec、`home` 误报 | RC1×3 + RC2 |
| F2 | #59 + #63 | `gh` 抛错、冒号形式、未闭合注释 | RC2×3 |
| F3 | #58 | W7 非空、W4 fail-closed 无测试 | RC1 + RC2 |
| F4 | #56 | `Auto-close` 推导、`PROJECTS_TOKEN` | RC1 + RC3 |
| F5 | #57 | 机械可执行过度承诺、逐字节相同 | RC1 + RC3 |
...
- [ ] F4 #56
- [ ] F5 #57
- [ ] 整理提交、推送、逐条回复并 resolve 评审 thread
