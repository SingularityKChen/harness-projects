...
| **Status** | 回答规划所有者是否**接受**该工作项完成（权威定义见 `docs/product/board-semantics.md`） |
...
`AGENTS.md` §8.3 规定代码 PR ≤ 1000 行、文档 ≤ 1500 行。这个约束涉及两个机制，闭环程度并不相同——本节最初把两者一并描述成「把 PR 体量约束从评审时前移到规划时」，issue #48 指出这个闭环声明对机制一不成立，此处更正：机制二是结构性闭环，机制一只是一个未与评审时度量**连通**的申报信号（完整理由见 `docs/product/board-semantics.md` §7）。

**机制一：`Size` 字段**（看板单选字段）是规划时的**人工信号**，不是控制：在动手之前声明预计 diff 量级，但申报值与评审时 `rule-checks size` 的机械度量彼此不连通——没有任何检查核对「申报 `XS` 的条目交付时是否真的只有约 100 行」。这不是遗漏，而是有意的权衡：连通需要读看板的 `Size` 字段，那需要 `PROJECTS_TOKEN`（至今不存在），会把一条当前离线、必需的检查降级成需要凭据的 advisory 检查，不划算。取值：
...
**机制二：sub-issue（这一半是真正的结构性闭环）**。一个 sub-issue = 一个批次 = 一个 PR。父条目只做协调，不占 PR。
...
| Item closed → Status = Done | **不开启** | issue 关闭由 `Closes #N` 在 PR 合并时自动触发，不是人直接做出的规划动作：2026-09-18 合并 8 个 PR 时实测到 8 次自动写入（`#14 #15 #16 #17 #18 #20 #32 #34` 全部被自动写成 `Done`，无人参与、无 actor 痕迹），证据见 issue #45。权威定义与完整推导见 `docs/product/board-semantics.md` |
...
secret 边界采用两段式触发：`pull_request_review` 只运行无权限、无 secret、无 checkout 的 signal workflow；默认分支上的特权 workflow 通过 `workflow_run` 消费。**消费者不读取任何 PR 上下文可改写的内容**：触发 run 的 PR 编号取自该 run 自身的关联（payload 的 `workflow_run.pull_requests`，为空时回退到 runs API），准入控制是否跳过由触发 run 的 job 结论判定——`skipped` 是有理由的 no-op，其余拿不到输入的情形一律非零失败。PR 生命周期使用 `pull_request_target`。特权 job 显式 checkout 默认分支，绝不执行 PR head；review actor 关联关系只限制是否产生信号，`Approved` 真值始终来自 snapshot 的 `reviewDecision`。所有 reconcile 全局串行且不相互取消：PR 编号在 job 运行后才可信可用，提前按 payload 分组既不可靠，取消已经开始的正确 reconcile 又可能丢失唯一一次写入。
...
- **Observation**：`workflow_run.pull_requests[0]` 不是 GitHub 对跨工作流传递 PR 编号给出的可靠契约；官方示例显式上传 PR 编号 artifact，而且现有 GraphQL 查询的 `first:20` 也会在关联较多时静默截断。
  **订正（2026-09-21）**：这条结论被评审推翻并已改回 run 关联：artifact 是「不可执行」但**可被同仓 PR 伪造**的输入，而 run↔PR 关联由 GitHub 记录、PR 无法改写。现在消费者从 `workflow_run.pull_requests` 取编号（为空时回退 runs API），并且不再有任何 artifact 下载步骤——删掉一个可伪造输入比给它加校验更强。截断风险仍由 `totalCount` 完整性校验覆盖。
  ...
  **订正（2026-09-21）**：artifact actions 已从两个 workflow 中删除，因此「固定 artifact action SHA」这条证据随之作废；`totalCount` 完整性校验仍然有效。
...
- **Decision**：Engineering 同步采用“无权 signal → 默认分支可信 reconcile → ack 后确认”的三段边界。
  **Rationale**：事件顺序、单个 reviewer 状态与当前聚合结论不是同一事实；让 PR head 代码接触用户级 Project PAT 又越过了最小权限边界。重新查询完整 snapshot 能处理乱序和过期事件，PR 编号取自触发 run 自身的关联以跨越权限边界，固定 concurrency group 保证 query/write 不交错，ack 校验则守住“attempted 不等于 confirmed”的不变量。
  **订正（2026-09-21）**：原决策写的是「PR 编号经 inert artifact 传递」。评审证明 artifact 可被同仓 PR 伪造（signal workflow 在 PR 上下文运行），因此改为从触发 run 的关联取编号并删除 artifact；判定表由纯函数 `classifySignalRun` / `selectAssociatedPullRequest` 承担并被契约测试钉住。
...
- 2026-09-21：按 PR #37 的两条未解决 P1 订正上一行——删除 artifact 传递，PR 编号改回触发 run 自身的关联，并把「准入跳过」与「拿不到输入」显式分开（前者是有理由的 no-op，后者非零失败）。原因：artifact 不可执行但可伪造，而 run↔PR 关联由 GitHub 记录；同时 `continue-on-error: true` 会把下载失败伪装成 no-op。
