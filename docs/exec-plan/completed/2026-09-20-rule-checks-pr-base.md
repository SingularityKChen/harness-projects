...
三个批次全部完成，验收项 1–8 与 10 通过，第 9 项需要合并后的真实事件。本分支的提交序列是：计划 → Batch 1（workflow + 脚本）→ Batch 2（契约测试新增 7 条）→ Batch 3（文档与计划回填）→ 登记 PR #98 与真实 CI 事件的报告证据 → 对抗验收后的 P2 补测（契约测试再增 2 条）→ 记录 `runs` API 复核陷阱并回填验收结论。这里刻意不写提交总数：本句自身就在其中一次提交里，写死的数字在写下它的那一刻就会过期——与 Surprises 9 的 `runs` 计数是同一类错误（第一次写成 6，既漏列又被自己的修正提交推翻）。
