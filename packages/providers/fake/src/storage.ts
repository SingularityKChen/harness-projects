...
export class MemoryStorage implements cap.Storage {
  ...
  async putEntity(record: domain.Entity): Promise<void> { upsert(this.data.entities, record, (e) => e.id === record.id) }
  ...
  async putRepository(record: cap.RepositoryRecord): Promise<void> { upsert(this.data.repositories, record, (r) => r.id === record.id) }
  ...
  async putExecutionRun(record: cap.ExecutionRunRecord): Promise<void> { upsert(this.data.runs, record, (r) => r.id === record.id) }
  ...
  async putRelation(workspaceId: domain.WorkspaceId, relation: domain.Relation): Promise<void> {
    upsert(this.data.relations, { workspaceId, relation }, (r) =>
  ...
  async putSyncCursor(record: cap.SyncCursorRecord): Promise<void> { upsert(this.data.cursors, record, (c) => c.bindingId === record.bindingId && c.scopeKey === record.scopeKey) }
