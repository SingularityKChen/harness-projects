/**
 * 关系图：系统事实边与谱系跳（issue #78 / AGENTS.md §1.1 不变量 5、6）。来源各有唯一落点：链路推进（显式命令拿到 provider ack）
 * 写 explicit 边；从 provider 读到的确定性事实写 deterministic 边，初始状态只能是 candidate；沿已记录边继续走得到的跳标 lineage。
 * 同一 (from, type, to) 三元组只保留一条：重复记录返回既有关系，绝不把 confirmed 降级回 candidate。工程产物的内部 id 由
...
export interface RelationRef { readonly from: EntityId; readonly to: EntityId; readonly type: RelationType }

export const EdgeProvenance = {
  Command: 'command', ProviderRead: 'provider_read', ChainSkeleton: 'chain_skeleton',
} as const
...
export interface ChainNode {
  readonly id: EntityId; readonly kind: EntityKind; readonly externalId: string | undefined
  readonly label: string | undefined; readonly observed: boolean; readonly detail: string | undefined
...
export interface DiscoveredEdge extends RelationRef { readonly artifact: ChainNode; readonly provenance: EdgeProvenance }

export interface RecordedEdge { readonly relation: Relation; readonly created: boolean }
...
export function asEntityId(raw: string): EntityId { return asBrandedId<EntityId>(raw) }
...
export async function recordEdges(context: CoreContext, edges: readonly DiscoveredEdge[]): Promise<readonly RecordedEdge[]> {
  ...
  for (const edge of edges) {
    ...
    if (existing !== undefined) { results.push({ relation: existing, created: false }); continue }
